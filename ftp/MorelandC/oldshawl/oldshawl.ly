...
melody = \notes \relative c'' {
	...
	c4 a8 bf c f4 f8 | e4 d8 bf g4. c8 | e d c a c b bf g | f2 ~ f4 
...
upper = \notes\relative c'' {
	...
	<d f>8.^\markup { \bold \large "Moderato" } <c ef>16 |
%% 1-4
	...
	\partial 4 <d f>8. <c ef>16 <bf d>4 <bf d,>8. <c ef,>16 <d f,>8 f,, <bf d> f |
	...
	\partial 4*3 r8 a <a f'> <a f'> <a ef' f>4\fermata \bar "|." 
...
lower = \notes\relative c {
	...
	\partial 4 r4 <<{bf2}\\{bf8 f' <bf d> f}>> bf,2 | ef c | f f |
...
SopMel = \notes \relative c'' { \set Staff.instrument = #"Sop."
	\key bf\major \time 4/4 
...
AltoMel = \notes \relative c'' { \set Staff.instrument = #"Alto"
	\key bf\major \time 4/4 \partial 4
...
TenMel = \notes \relative c' { \set Staff.instrument = #"Tenor"
...
BassMel = \notes \relative c { \set Staff.instrument = #"Bass"
...
\score {
	<<
		\context Voice = melA { \autoBeamOff \SopMel }
			\lyricsto melA \new Lyrics \Chorus

		\context Voice = melB { \autoBeamOff \AltoMel }
					
		\context Voice = melC { \autoBeamOff \TenMel }
			\lyricsto melC \new Lyrics \Chorus

		\context Voice = melD { \autoBeamOff \BassMel }
			\lyricsto melD \new Lyrics \BassChorus	
				
      \context PianoStaff <<
	  \context Staff = upper \ChorusUp
	  \context Staff = lower <<
	      \clef bass
	      \ChorusDown
	  >>
