...
theFlutes = \notes \relative d'' {
	...
	<< { \times 2/3 { d8_.[_( d_. d_.^]_)\! } d16_.[_(\> d_. d_. d_.]_) d4\trill
		\bar "" \grace { csharp16[ d]\! } \bar "|" } \\ { R1*3/4 } >>
	<< { bflat8[( a g) fsharp( g eflat]) } \\ { R1*3/4 } >>
	<< { d8[( c a fsharp]) d4( } \\ { R1*3/4 } >>
	<< { g8) r r4 r } \\ { R1*3/4 } >>
	R1*3/4^\markup { \bigger rit. }
	
	R1*3/4*34
	
	<< { r16 g'\mf[( bflat eflat] d8) d4-> d8 } \\ { R1*3/4 } >>
	<< { r16 a[( d g] fsharp8) fsharp4-> fsharp8 } \\ { R1*3/4 } >>
	<< { \stemDown g8[ g,]~ g[ g( g') a,]( \stemBoth } \\ { R1*3/4 } >>
	<< { \stemDown a'8.)[ fsharp16] d[ a fsharp d]~ \stemBoth d[ d,8 d16] } \\ { R1*3/4 } >>
	...
	R1*3/4
	< aflat c >2.\mf
	R1*3/4^\markup { \bigger rallent. }
	< aflat dflat >2.\p
	R1*3/4
	< d e >2.
	R1*3/4
	R1*3/4^\markup { \bigger {riten. molto} }
	R1*3/4
	r1*3/4\fermata
