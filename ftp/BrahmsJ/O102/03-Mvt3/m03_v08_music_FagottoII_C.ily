%  work         : Double Concerto Op102
%  typesetter   : Sébastien MANEN
%  date         : Thursday 20 April 2023, 19:50
%###############################################################################
%#                          M U S I C   S E C T I O N                          #
%###############################################################################
musicFagottoIIMvtIII = \relative c {
	\clef bass
	\key a \minor
%	\transposition a
% Bars 1 to 5
	R2
	r4 r8 c-.\p
	a'4 r8 c,-.
	a'4 r8 e-.
	c'4 r8 e,-.
% Bars 6 to 10
	c'4 r8 \clef tenor g-.
	e'4( dis
	e8 b a4)
	gis r4
	R2*19
% Bars 11 to 15
	
% Bars 16 to 20
	
% Bars 21 to 25
	
% Bars 26 to 30
	
	
	
	R2*2
	\clef bass
% Bars 31 to 35
	a2\pp~
	a
	d,(
	g
	c,8-.) r r4
% Bars 36 to 40
	R2*3
	
	\mark \default
	e,2-\tweak X-offset #-1.5 \ff
	a16-. b-. c8-. a16-. c-. f8-.
% Bars 41 to 45
	a,16-. d-. gis8-. a,16-. e'-. c-. b-. 
	a8-. e-. f-. e-. 
	a-. e-. f-. e-. 
	c'-. g-. a-. g-. 
	c-. g-. aes-. g-. 
% Bars 46 to 50
	e'->-.[ c-.] c->-. a-. 
	a->-.[ f-.] f->-. d-. 
	\beamOffset #'(0 . 1) c' a e' e, 
	a r r4
	R2*15 
% Bars 51 to 55
	
% Bars 56 to 60
	
% Bars 61 to 65
	
	
	
	
	\clef bass g8\f-. g'-. g,-. g'-. 
% Bars 66 to 70
	g,-. g'-. g,-. g\p~\noBeam
	g g4 g8~
	g g4 g8
	R2*15
% Bars 71 to 75
	
% Bars 76 to 80
	
% Bars 81 to 85
	
	
	
	r4 d'~ \bar "||"
	\time 3/4 d2.~
% Bars 86 to 90
	d4( g2) \bar "||"
	\time 4/4 R1
	b,4(\< c cis d8 g,)\! \bar "||"
	\timeMvtIII a8-.\f r r4
	d8-. r r4
% Bars 91 to 95
	g,8-. r r4
	R2*2
	
	r4 c8(\p\< d\!
	f\> e)\! r4
% Bars 96 to 100
	r c8(\< d\!
	f\> e)\! r4
	R2*3
	
	\mark \default
% Bars 101 to 105
	R2*18
	
	
	
	
% Bars 106 to 110
	
% Bars 111 to 115
	
% Bars 116 to 120
	
	
	\bar "||" \key d \minor
	R2*18
	
% Bars 121 to 125
	
% Bars 126 to 130
	
% Bars 131 to 135
	
% Bars 136 to 140
	
	\clef bass bes'4..\ff bes16
	bes4.. bes,16
	f8. g16 a4
	R2*2
% Bars 141 to 145
	
	r8. e''16 c8[ s16 a] 
	f4 g
	e f8. c16
	e4 a,
% Bars 146 to 150
	R2*2
	\clef tenor \mark \default
	r4 r8 s
	s2
	s2
% Bars 151 to 155
	s2
	r4 r8 bes'8
	c c4 des8(
	f,) f4 g8(
	aes) aes4 g8(
% Bars 156 to 160
	\stemOffset #3 bes) \stemOffset #2 bes4 \stemOffset #3 bes8( 
	a!) r8 r4
	r r8 e8(
	a) a4 gis8(
	a) a4 d8(\>
% Bars 161 to 165
	s) s4\! d8
	a a4 g8
	s s4 \voiceOne \noFlag e8_(
	\noFlag f) f4 r8
	R2*11
% Bars 166 to 170
	
% Bars 171 to 175
	
% Bars 176 to 180
	r4 r8 \clef bass d(\f
	a'4) r8 bes,(
	f'4) r
	R2 \mark \default
	R2*16
% Bars 181 to 185
	
% Bars 186 to 190
	
% Bars 191 to 195
	
% Bars 196 to 200
	r4 r8. a16\ff
	a8( f!16) f( d) d( cis) cis(
	f) f( d) d( bes) bes( a) a
	g4.. g16
	bes4.. bes16
% Bars 201 to 205
	f8. g16 a4
	r8. a16-. f8-.[ r16 d-.]
	bes4 c
	a' bes8. f16
	a4 d,
% Bars 206 to 210
	d'2\sf
	d\sf
	d4-> d->
	d8-. r r4
	a2\sf
% Bars 211 to 215
	a\sf
	a4-> a->
	a8-. r r4
	\voiceTwo e'8\fp f e d
	e d c b
% Bars 216 to 220
	c cis d b
	c( d e e,) \bar "||" \mark \default \key a \minor
	a8\p r r4
	R2*19
% Bars 221 to 225
	
% Bars 226 to 230
	
% Bars 231 to 235
	
% Bars 236 to 240
	
	\mark \default
	\clef tenor c'4( bes
	a b!)~
	b8 r r4
% Bars 241 to 245
	r8 des( c bes)
	a r r4
	R2*3
	
	
% Bars 246 to 250
	R2*4 
	
	
	
	R2*5
% Bars 251 to 255
	
	
	
	
	\clef tenor e'2_\mfcresc
% Bars 256 to 260
	d
	b \clef bass
	a,16-.-\tweak X-offset #-1.5 \ff b-. c8-. a16-. c-. f8-.
	a,16-. d-. gis8-. a,16-. e'-. c-. b-.
	a8-. e-. f-. e-. 
% Bars 261 to 265
	a-. e-. f-. e-. 
	c'-. g-. a-. g-. 
	c-. g-. aes-. g-. 
	e'->-.[ c-.] c->-. a-. 
	a->-.[ f-.] f->-. d-. 
% Bars 266 to 270
	c'-. a-. e'-. e,-. 
	a'-.[ a,-.] c'-. c,-. 
	d'-. r r4
	R2*5
% Bars 271 to 275
	
	
	\bar "||"
	\key a \major R2*8
% Bars 276 to 280
	
% Bars 281 to 285
	
	r4 e,,\mf\<(
	g) fis(\!
	b)\> d\!
	cis8([ a)] d(-. cis-.)
% Bars 286 to 290
	b( fis) fis4~
	fis b
	r b~ \bar "||"
	\time 3/4 b2.~
	b4( e2) \bar "||"
% Bars 291 to 295
	\time 4/4 R1
	\shape #'((0 . 0)(0 . 1)(0 . 1)(0 . 0)) Slur gis,4(\< a ais b8 e)\! \bar "||"
	\timeMvtIII fis,8-.\f r r4
	b8-. r r4
	e,8-. r r4
% Bars 296 to 300
	R2 \bar "||"
	a'8_\pdolce g fis f
	e[ d cis] cis(
	d cis4) cis8(
	bis cis4.)~
% Bars 301 to 305
	cis \clef tenor fis8(
	g fis4) fis8(
	\voiceOne \noFlag f fis4.)_~
	fis4._( \noFlag e8)
	r \voiceTwo gis(\< a\> gis\!)
% Bars 306 to 310
	r8 b(\p a gis) 
	r gis(\< a\> gis)\! 
	r8 d'(\p cis b)
	r8 a4 d8(
	cis b a gis
% Bars 311 to 315
	eis fis4) fis'8(
	e!\> d cis b)\!
	cis\p r r4
	R2
	r4 r8 fis(
% Bars 316 to 320
	e\> d b d,)\! \clef bass
	\partCombineApart cis4\p d\<
	dis e\! \partCombineAutomatic
	cis4\f fis,
	<<b2 {s4\sf\> s\!} >>
% Bars 321 to 325
	cis4-\tweak X-offset 0.5 \p r
	b4.-\tweak X-offset #2 \pp\> r8\! 
	b4.-\tweak X-offset #2 \pp\> r8\! \bar "||"
	r4 r8 c'(\f
	a fis dis c)
% Bars 326 to 330
	a4. d8-.
	cis-. r e-. r
	a,-.-\tweak X-offset #1 \p r r4
	R2*7
% Bars 331 to 335
	
% Bars 336 to 340
	a8-.-\tweak X-offset #-2.5 \f r r4
	R2
	a'8-. r r4
	a8-. r r4
	a,2\fermata \bar "|."
}
