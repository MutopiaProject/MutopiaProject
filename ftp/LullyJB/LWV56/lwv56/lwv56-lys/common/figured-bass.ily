...
#(define (fig-seq->markup-seq seq-music)
  ...
  (define (alteration->markup alteration)
    (markup #:teeny #:raise (if (= alteration FLAT)
                                 0.3
                                 0.6)
     #:musicglyph (string-append "accidentals." 
                                 (number->string alteration))))
  ...
  (define (fig->markup fig-music)
   (let* ((fig  (ly:music-property fig-music 'figure))
          ...
          (fig-markup (cond ((markup? fig) fig)
                            ((number? fig)
                             (if (eq? #t (ly:music-property fig-music 'diminished))
                                 (markup #:teeny #:slashed-digit fig)
                                 (markup #:teeny #:number (number->string fig 10))))
                            ((not (null? acc)) (markup #:teeny ""))
                            (else (markup #:teeny #:strut)))))
    (list (make-music 'TextScriptEvent
            'direction -1
            'text (markup (cond ((number? acc)
                                 (markup fig-markup (alteration->markup acc)))
                                ((eq? #t (ly:music-property fig-music 'augmented))
                                 (markup fig-markup #:number "+"))
                                (else fig-markup))))
