...
(define (write-page-tweaks pages)
  ...
  (define (dump-all-tweaks)
    (let* ((paper (ly:paper-book-paper (page-property  (car pages) 'paper-book)))
	 ...
	 (display "}" out-port))
       tweaks)
      (format out-port "~%#(set! precomputed-number-of-lines-per-page~%  '~a)~%"
              (map (lambda (page)
                     (length (page-property page 'lines)))
                   pages))
      (close-port out-port)))
...
(define-public (naive-page-breaks book-lines paper-book)
  ...
  (define (get-pages lines)
    ...
    (let ((book-pages (get-pages book-lines)))
      ;; re-order page lines
...
(define-public precomputed-number-of-lines-per-page #f)

(define-public (precomputed-page-breaks book-lines paper-book)
  "Return pages as a list starting with 1st page. Each page is a 'page Prob.
Pages breaks are previously computed in variable
`precomputed-number-of-lines-per-page', a list of numbers, each number
designating the number of lines in each page."
  
  ;; (define (make-new-page last?) ...)
  (define make-new-page
    (let* ((paper (ly:paper-book-paper paper-book))
           (page-alist (layout->page-init paper))
           (current-page-num (1- (ly:output-def-lookup paper 'first-page-number))))
  ...
  (define (push-lines! page number lines)
    "Put `number' of `lines' on `page'."
    (page-set-property! page 'lines
                        (loop for line in lines
                              repeat number
                              collect (fix-line-extent! line)))
    (page-set-property! page 'configuration
                        (loop for previous-line in (page-property page 'lines)
                              for position = 0.0
                              then (+ position
                                      (after-line-padding previous-line)
                                      (line-height previous-line))
                              repeat number
                              collect position)))
                                             
  (ly:message (_ "Using precomputed page breaks... "))
  (let ((expected-nb (reduce + 0 precomputed-number-of-lines-per-page))
        (real-nb (length book-lines)))
    (if (not (= expected-nb real-nb))
        (format #t "~%Warning: number of lines (~a) different from expected (~a)."
                real-nb expected-nb)))
  (let* ((nb-lines (length book-lines))
         (book-pages (loop for rest-lines = book-lines
                           then (list-cdr-ref rest-lines line-number)
                           for line-number in precomputed-number-of-lines-per-page
                           for last? = (>= line-number (- nb-lines nb-treated))
                           for nb-treated = 0 then (+ nb-treated line-number)
                           while (< nb-treated nb-lines)
                           collect (let ((page (make-new-page last?)))
                                     (push-lines! page line-number rest-lines)
                                     page))))
    (let ((expected-nb (length precomputed-number-of-lines-per-page))
          (real-nb (length book-pages)))
      (if (not (= expected-nb real-nb))
          (format #t "~%Warning: number of pages (~a) different from expected (~a)."
                  real-nb expected-nb)))
    (for-each page-stencil book-pages)
    (post-process-pages (ly:paper-book-paper paper-book)
                        book-pages)
