...
sopDynamics = {
	...
	s1.*3
	s2 s2 s2 s1.
	s2\once \override TextScript #'script-priority = #-100 s2^\turn^\markup{\tiny \smaller \smaller \natural} s2
...
lhOne = {
	...
	%Page 1, Line 2
	\slurDown
	aes8 des'8 f'8. aes'16 aes'8 aes8 g'8 aes8 f'8 aes8 ees'8 aes8 \change Staff=lh \stemUp |
	...
	%Page 3, Line 2
	aes8( bes8 \change Staff=rh \stemDown d'8. f'16) f'8( d'8 \change Staff=lh \stemUp bes8 aes8^\markup{\translate #(cons -.8 2.7)\italic "rit."}) aes8( bes8 \change Staff=rh \stemDown aes'8 d'8) \change Staff=lh \stemUp |
...
\score {
	<<
		\context Staff = melStaff {
			<<
				\context Voice = mel {\transpose c \target \sopNotes }
		...
		\context PianoStaff <<
			\context Staff = rh {
				<<
					\context Voice = rhA {\transpose c \target \rhOne }
					\context Voice = rhB {\transpose c \target \rhTwo }
				>>
			}
			\context Staff = lh
			{
				<<
					\context Voice = lhA {\transpose c \target \lhOne }
					\context Voice = lhB {\transpose c \target \lhTwo }
